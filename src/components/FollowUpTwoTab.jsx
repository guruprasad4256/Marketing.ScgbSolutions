...
import { 
  ...
  Inbox, Eye, Minus, Flame
} from 'lucide-react';

const FollowUpTwoTab = ({ 
  ...
  const handleQuickSubmit = async () => {
    ...
    // Fires Template 179 specifically for the 2nd Follow-up
    await onQuickSend(quickName, quickEmail, "179");
    setQuickName(''); 
    setQuickEmail('');
  };
  ...
  // --- FUNNEL MATH (Specific to Follow-up 2) ---
  const funnel = useMemo(() => {
    const today = new Date().toISOString().split('T')[0];
    const oneWeekAgo = new Date(); oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);

    const baseData = data.filter(lead => {
      // Specifically looking for the 2nd follow-up string
      if (!lead.campaignReport || !lead.campaignReport.includes('Follow-up 179')) return false;
  ...
  // Updated color scheme to Orange for the 2nd Follow-up
  const statConfig = [
    { label: 'F-Up 2 Sent', val: funnel.sent, color: 'orange', icon: Flame, key: 'SENT', activeClass: 'ring-orange-500 bg-orange-50/50', text: 'text-orange-600' },
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 1. QUICK FOLLOW-UP 2 LAUNCHPAD */}
      <Card className="border-orange-200 bg-orange-50/30 shadow-sm overflow-hidden">
        <div className="bg-orange-500 px-4 py-1.5 text-[10px] font-bold text-white uppercase tracking-widest flex items-center gap-2">
          <Flame className="h-3 w-3" /> Quick Follow-up 2 Launch
        </div>
        <CardContent className="p-4 flex flex-col lg:flex-row items-end justify-between gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input 
                  ...
                  className="pl-9 h-9 bg-white text-xs border-slate-200 focus:ring-orange-500"
                  value={quickName}
          ...
          <Button 
            ...
            className="bg-orange-500 hover:bg-orange-600 text-white h-9 px-8 text-xs font-bold gap-2 mb-[1px] whitespace-nowrap"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <Button 
          ...
          onClick={onSync} 
          disabled={isSyncing} 
          className="h-full border-2 border-dashed border-slate-200 flex flex-col gap-1 items-center justify-center min-h-[90px] bg-white hover:bg-orange-50 hover:border-orange-400 transition-colors"
        >
          {isSyncing ? <Loader2 className="h-5 w-5 animate-spin text-orange-500" /> : <Calendar className="h-5 w-5 text-slate-400" />}
          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">Sync Reports</span>
        </Button>
      </div>

      {/* 3. FOLLOW-UP 2 ACTIVITY LOG GRID */}
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader className="bg-slate-50">
              <TableRow>
                <TableHead className="text-[10px] font-black pl-6 py-4 uppercase text-slate-500">Hot Lead</TableHead>
              ...
              {displayedCampaignData.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-12 text-slate-400 text-xs italic">
                    No stage 3 lead activity found.
                  </TableCell>
                </TableRow>
              ) : (
                displayedCampaignData.map(lead => {
                  ...
                  return (
                    <TableRow key={lead.id} className="hover:bg-orange-50/50 cursor-pointer transition-colors group" onClick={() => onSelectContact(lead)}>
                      ...
                      <TableCell>
                        <span className="text-[9px] font-black uppercase px-2 py-0.5 rounded border text-orange-700 bg-orange-50 border-orange-200">
                          {p.source}
                        </span>
                      </TableCell>
